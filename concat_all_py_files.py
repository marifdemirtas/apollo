...
def get_files_cyber():
    with open('py_file_list.txt', 'r') as f:
...
def create_concat_cyber():
    ...
    with open('all_module_py_files_concat_cyber.py','w') as wfd:
        for f in files:
            with open(f, 'r') as fd:
                shutil.copyfileobj(fd, wfd)

def get_files():
    with open('py_modules_list.txt', 'r') as f:
        files = [d[:-1] for d in f.readlines()]
        return files
...
def create_concat():
    files = get_files()
    with open('all_module_py_files_concat.py','w') as wfd:
        for f in files:
            with open(f, 'r') as fd:
                shutil.copyfileobj(fd, wfd)
...
def add_logging():
    ...
    with open('all_module_py_files_concat.py','r') as wfd:
        full_text = wfd.read()
        replace_start = 0
    ...
    with open('all_module_py_files_concat_logged.py','w') as wfd:
        wfd.write(full_text)

def read():
    files = get_files()
    print()
    with open('all_module_py_files_concat_logged.py','r') as wfd:
